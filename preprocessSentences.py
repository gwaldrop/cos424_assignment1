...
def tokenize_corpus(path, train=True):
  ...
  for line in lines:
    ...
    # Remove negation
    #for w in range(len(tokens) - 2):
    #  if (tokens[w] == 'not') or ('n\'t' in tokens[w]):
    #    del tokens[w + 1]
    ...
    # Bigram capture
    bigrams = [tokens[index] + ' ' + tokens[index + 1] for index in range(len(tokens) - 1)]
...
def main(argv):
  ...
  # Get test bag of words
  testtxt = path+"/test.txt"
  (tstdocs, tstclasses, tstsamples) = tokenize_corpus(testtxt, train=False)
  tstbow = find_wordcounts(tstdocs, vocab)
  with open(path+"/tst_bag_of_words_"+str(word_count_threshold)+".csv", "wb") as f:
    writer = csv.writer(f)
    writer.writerows(tstbow)

  outfile= open(path+"/tst_classes_"+str(word_count_threshold)+".txt", 'w')
  outfile.write("\n".join(tstclasses))
  outfile.close()
